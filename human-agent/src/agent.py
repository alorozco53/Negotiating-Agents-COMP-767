...
class RnnAgent(Agent):
    ...
    def read(self, inpt):
        ...
        inpt = self._encode(inpt, self.model.word_dict).to('cuda:0')
        self.lang_h = self.lang_h.to('cuda:0')
        #print("Locations test", Variable(inpt).device, self.lang_h.device, self.ctx_h.device)
    ...
    def write(self, max_words=100):
        #print("Locations in wrint", self.lang_h.device, self.ctx_h.device)
        acts, outs, self.lang_h, lang_hs = self.model.write(self.lang_h , self.ctx_h , max_words, self.args.temperature)
        self.lang_hs.append(lang_hs)
        you = self.model.word2var('YOU:').to('cuda:0')
        self.words.append(you.unsqueeze(0))
        self.words.append(outs)
        self.sents.append(torch.cat([you.unsqueeze(1), outs], 0))
        #assert (torch.cat(self.words).size(0) == torch.cat(self.lang_hs).size(0))
    ...
    def _make_idxs(self, sents):
        ...
        for sent in sents:
            ...
            ln = torch.Tensor(1).fill_(sent.size(0) - 1).long().to('cuda:0')
            lens.append(ln)
            idx = torch.Tensor(sent.size(0), 1, 1).fill_(-1).long().to('cuda:0')
    ...
    def _choose(self, sample=False):
        ...
        for i in range(self.domain.selection_length()):
            ...
            idxs = Variable(torch.Tensor(idxs).long()).to('cuda:0')
            #print("device for this is", idxs.device)
