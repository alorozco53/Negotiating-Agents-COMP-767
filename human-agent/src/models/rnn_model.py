...
class RnnModel(nn.Module):
    ...
    def read(self, inpt, lang_h, ctx_h, prefix_token='THEM:'):
        ...
        prefix = self.word2var(prefix_token).unsqueeze(0).to('cuda:0')
    ...
    def write(self, lang_h, ctx_h, max_words, temperature,
        ...
        ctx_h = ctx_h.squeeze(1)
        inpt = None if resume else self.word2var('YOU:').to('cuda:0')
        ...
        for _ in range(max_words):
            if inpt is not None:
                ...
                lang_h = lang_h.to('cuda:0')
                #print("Locations are ", inpt_emb.device, lang_h.device)
                lang_h = self.writer(inpt_emb.to('cuda:0'), lang_h)
