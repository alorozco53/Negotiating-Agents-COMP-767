...
class SelectionModule(nn.Module):
    ...
    def forward(self, q, hs, lens, rev_idxs, hid_idxs):
        ...
        q = q.to('cuda:0')
        #print("All stuff", q.device, hs, lens, rev_idxs, hid_idxs)
...
class SelectionModel(nn.Module):
    ...
    def forward_inpts(self, inpts, ctx_h):
        ...
        for i, inpt in enumerate(inpts):
            inptc = inpt[0].to('cuda:0')
            speaker_emb = self.speaker_encoder(inptc).unsqueeze(0)
            inpt_emb = self.word_encoder(inpt.to('cuda:0'))
            inpt_emb = self.dropout(inpt_emb)
            pos = Variable(torch.Tensor([min(self.len_cutoff, len(inpts) - i) - 1]).long()).to('cuda:0')
    ...
    def forward(self, inpts, lens, rev_idxs, hid_idxs, ctx):
        ...
        #print("device type here is", inpts, ctx_h)
