...
class Attention(nn.Module):
    ...
    def forward(self, q, v, mask=None):
        ...
        if mask is not None:
            logit = torch.add(logit.to('cuda:0'), Variable(mask).to('cuda:0'))
...
class HierarchicalAttention(ChunkedAttention):
    ...
    def forward_word_rnn(self, rnn, bsz, inpts, lens, rev_idxs, hid_idxs):
        ...
        for inpt, ln, rev_idx, hid_idx in zip(inpts, lens, rev_idxs, hid_idxs):
            #print("Location of input stuff is", self.zero_h(bsz).device, inpt.device)
            h, _ = rnn(inpt, self.zero_h(bsz).to('cuda:0'))
            hs.append(h)
        return hs
    ...
    def forward_sent_attn(self, query, word_hs):
        ...
        if len(word_hs) == 0:
            ...
            sent_h, _ = self.sent_rnn(word_hs, self.zero_h(bsz, n=2).to('cuda:0'))
